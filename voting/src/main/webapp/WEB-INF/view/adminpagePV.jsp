...
<table id="customers" border=2>
  ...
  <c:forEach items="${lv}" var="elements">    
  <tr>
     <td><c:out value="${elements.voterId}" />
      <td><c:out value="${elements.voterName}" />
...
Select ID:<select name="appv" >
<c:forEach items="${lv}" var="elementss"> 
<option  value="${elementss.voterId}">${elementss.voterId}</option>
</c:forEach>
</select>
